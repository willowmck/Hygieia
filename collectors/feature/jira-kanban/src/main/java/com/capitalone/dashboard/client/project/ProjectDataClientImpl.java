...
public class ProjectDataClientImpl implements ProjectDataClient {
    ...
    private List<BasicProject> projects = new ArrayList<>();
	 ...
	 */
    @Override
	public synchronized int updateProjectInformation() {
		int count = 0;
    ...
    @Override
    public synchronized List<BasicProject> getProjects() {
        List<BasicProject> projectsCopy = new ArrayList<>();
	...
	private void updateMongoInfo(List<BasicProject> currentPagedJiraRs) {
		...
		if (currentPagedJiraRs != null) {
			ObjectId jiraCollectorId = featureCollectorRepository.findByName(FeatureCollectorConstants.JIRA_KANBAN).getId();
