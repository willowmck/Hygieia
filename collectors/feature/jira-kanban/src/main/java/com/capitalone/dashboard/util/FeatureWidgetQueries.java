...
public class FeatureWidgetQueries {
	...
	/**
	 * Retrieves source system queries based on the query name.
	 * 
	 * @param project
	 *            The project name to pull data from.
	 * @param queryName
	 *            The source system query name (without the file type).
	 * @return A given source system query, in String format.
	 */
    public String getStoryQuery(String project, String queryName) {
        ST st = folder.getInstanceOf(queryName);
        st.add("project", project);
        String query = st.render();
